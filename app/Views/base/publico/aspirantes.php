                            ...
                            <label class="block text-sm font-medium text-gray-700 mb-1">
                                ...
                                <span class="block text-xs font-normal text-gray-500 mt-1">
                                    ...
                                    <a href="https://www.gob.mx/curp" target="_blank"
                                        class="text-blue-600 hover:underline">www.gob.mx/curp</a>.
                                    Asegúrate de que sea la version digital, no deben ser fotos o capturas de pantaala en
                                    formato PDF.
                                </span>
                            </label>
                            <input type="file" name="curp" accept=".pdf" required
                                class="w-full border rounded p-2 mt-1 text-sm">
                        </div>
                        <button type="submit"
                            class="bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700 transition">Analizar
                            CURP</button>
                    </form>

                    <!-- Formulario paso 2: Continuar registro -->
                <?php else: ?>
                    <form action="<?= base_url('guardar-aspirante') ?>" method="post"
...
<script>
    // DEFINICIÓN de la función antes de usarla ESTO ES NUEVO NUEVO 
    ...
    function cargarSedes(selectId) {
        $.ajax({
            ...
            success: function (data) {
                ...
                $select.empty().append('<option value="">Selecciona una sede</option>');
                if (data.length === 0) {
                    console.warn("No se recibieron sedes.");
                }
                data.forEach(function (sede) {
                    $select.append('<option value="' + sede.id_sede + '">' + sede.nombre_sede + '</option>');
                });
...
<script>
    // DEFINICIÓN de la función cargarCarreras
    function cargarCarreras(selectId) {
        $.ajax({
            url: '<?= base_url('getCarreras') ?>',
            method: 'GET',
            dataType: 'json',
            success: function (data) {
                const $select = $(selectId);
                $select.empty().append('<option value="">Selecciona una carrera</option>');
                if (data.length === 0) {
                    console.warn("No se recibieron carreras.");
                }
                data.forEach(function (carrera) {
                    $select.append('<option value="' + carrera.id_carrera + '">' + carrera.nombre + '</option>');
                });
            },
            error: function (xhr, status, error) {
    ...
    $(document).ready(function () {
        ...
        cargarCarreras('#carrera_alt');
    });
</script>


<script>
    function cargarCarrerasPorSede(sedeId, selectId) {
        $.ajax({
            url: '<?= base_url('getCarrerasPorSede') ?>/' + sedeId,
            method: 'GET',
            dataType: 'json',
            ...
            error: function (xhr, status, error) {
                console.error("Error al cargar carreras por sede:", error);
            }
        });
    }
    ...
    $('#sede').on('change', function () {
        const sedeId = $(this).val();
        cargarCarrerasPorSede(sedeId, '#carrera');
    });

    $('#sede_alt').on('change', function () {
        const sedeId = $(this).val();
        cargarCarrerasPorSede(sedeId, '#carrera_alt');
    });
</script>
