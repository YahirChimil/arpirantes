...
<?php endif; ?>
    ...
    <a class="text-2xl font-bold text-gray-800 mb-6">
        conserva es documentacion una vez revisada la tendras que entregar en servicios escolares
             </a>
        ...
        <?php foreach ($documentos as $documento): ?>
            <div class="mb-6 p-4 border rounded bg-gray-50 shadow-sm">
                ...
                <?php if ($docSubido): ?>
                    ...
                    <form class="mt-3">
                        <input type="file" class="mb-2 cursor-not-allowed" disabled>
                ...
                <?php else: ?>
                    ...
                    <form action="<?= base_url('aspirante/subir_documento') ?>" method="post" enctype="multipart/form-data" class="mt-3">
                        <?= csrf_field() ?>
                        <input type="hidden" name="documento_id" value="<?= $documento['id'] ?>">

                        <input type="file" name="archivo" accept="application/pdf" class="mb-2" required>
                        <button type="submit"
                                class="flex items-center space-x-1 bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 stroke-current" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
